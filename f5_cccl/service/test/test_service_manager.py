...
class TestServiceConfigDeployer:
    ...
    def setup(self):
        ...
        svcfile = 'f5_cccl/schemas/tests/ltm_service.json'
        with open(svcfile, 'r') as fp:
            self.service = json.loads(fp.read())

        config_reader = ServiceConfigReader(self.partition)
        self.desired_config = config_reader.read_ltm_config(self.service)
    ...
    def get_created_objects(self, ltm_service_manager, obj_type):
        """Return list of created objects."""
        deployer = ltm_service_manager._service_deployer
        deployer._create_resources = Mock(return_value=[])

        ltm_service_manager.apply_ltm_config(self.service)
    ...
    def get_updated_objects(self, ltm_service_manager, obj_type):
    ...
    def get_deleted_objects(self, ltm_service_manager, obj_type):
    ...
    def test_app_services(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, ApplicationService)
        assert 1 == len(objs)
        assert objs[0].name == 'MyAppService0'

        # Should update one app service
        self.service['iapps'][0]['name'] = 'MyAppService'
        objs = self.get_updated_objects(ltm_service_manager, ApplicationService)
        assert 1 == len(objs)
        assert objs[0].name == 'MyAppService'

        # Should delete two app services
        self.service['iapps'] = []
        objs = self.get_deleted_objects(ltm_service_manager, ApplicationService)
        assert 2 == len(objs)
    ...
    def test_virtual_servers(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, VirtualServer)
        assert 1 == len(objs)
        assert objs[0].name == 'vs1'

        # Should update one Virtual Server
        self.service['virtualServers'][0]['name'] = 'virtual2'
        objs = self.get_updated_objects(ltm_service_manager, VirtualServer)
        assert 1 == len(objs)
        assert objs[0].name == 'virtual2'

        # Should delete one Virtual Server
        self.service['virtualServers'] = [] 
        objs = self.get_deleted_objects(ltm_service_manager, VirtualServer)
        assert 1 == len(objs)
    ...
    def test_pools(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, Pool)
        assert 1 == len(objs)
        assert objs[0].name == 'pool2'

        # Should update one Pool
        self.service['pools'][0]['name'] = 'pool1'
        objs = self.get_updated_objects(ltm_service_manager, Pool)
        assert 1 == len(objs)
        assert objs[0].name == 'pool1'

        # Should delete one Pool
        self.service['pools'] = [] 
        objs = self.get_deleted_objects(ltm_service_manager, Pool)
        assert 1 == len(objs)
    ...
    def test_monitors(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, HTTPMonitor)
        assert 1 == len(objs)
        assert objs[0].name == 'myhttp'

        # Should update one Monitor
        self.service['monitors'][0]['name'] = 'mon_http'
        objs = self.get_updated_objects(ltm_service_manager, HTTPMonitor)
        assert 1 == len(objs)
        assert objs[0].name == 'mon_http'

        # Should delete one Monitor
        self.service['monitors'] = [] 
        objs = self.get_deleted_objects(ltm_service_manager, HTTPMonitor)
        assert 1 == len(objs)
    ...
    def test_internal_data_groups(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, InternalDataGroup)
        assert 1 == len(objs)
        assert objs[0].name == 'test-dgs'

        # Should update one Data Group
        self.service['internalDataGroups'][0]['name'] = 'test-dg'
        objs = self.get_updated_objects(ltm_service_manager, InternalDataGroup)
        assert 1 == len(objs)
        assert objs[0].name == 'test-dg'

        # Should delete one Data Group
        self.service['internalDataGroups'] = [] 
        objs = self.get_deleted_objects(ltm_service_manager, InternalDataGroup)
        assert 1 == len(objs)
    ...
    def test_irules(self, ltm_service_manager):
        ...
        objs = self.get_created_objects(ltm_service_manager, IRule)
        assert 1 == len(objs)
        assert objs[0].name == 'https_redirect'

        # Should update one iRule
        self.service['iRules'][0]['name'] = 'https_redirector'
        objs = self.get_updated_objects(ltm_service_manager, IRule)
        assert 1 == len(objs)
        assert objs[0].name == 'https_redirector'

        # Should delete one iRule
        self.service['iRules'] = [] 
        objs = self.get_deleted_objects(ltm_service_manager, IRule)
        assert 1 == len(objs)
